...
def main():
    ...
    os.mkdir('img')
    try:
        max_pages_num = max_pages()
        ranges_all = ranges_for_parser(max_pages_num, 20)
        print(f"Total amount of pages is {max_pages_num}")

        for ranges in ranges_all[32:]:
            dataset = []
            links_list = []
            num = 0
            print(f"Parsing from {ranges[0]} to {ranges[1]} page")
            args1 = [os.getenv("URL").format(os.getenv("WOJEWODZTWO"), n) for n in range(ranges[0], ranges[1])]
